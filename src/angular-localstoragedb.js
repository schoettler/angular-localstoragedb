/*
  ...
  https://github.com/schoettler

	Angular localStorageDB v 1.0.0
	A simple database layer for localStorage

  Forked from:
  Kailash Nadh (http://nadh.in) https://github.com/knadh/localStorageDB
	License	:	MIT License
*/
...
(function() {
  ...
  angular
    .module('localstoragedb', [])
    .provider('LocalStorageDB', localstoragedb);
  ...
  function localstoragedb() {
    ...
    function setDefaultDatabase(name, version) {
    ...
    // Instance of the Factory method
    ...
    if (db.version == null || db_version > db.version) {
      ...
      return {
        ...
        insertOrUpdate: function(table_name, query, data) {
          ...
          if(result_ids.length === 0) {
            ...
            for(var n = 0; n < result_ids.length; n++) {
              update(table_name, result_ids, populate);
            }
        ...
        // select rows
        query: function(table_name, query, columns, limit, start, sort, distinct) {
          tableExistsWarn(table_name);
          var result_ids = [];

          if(!query) {
            // no conditions given, return all records
            result_ids = getIDs(table_name, limit, start);
          } else if(typeof query === 'object') {
            // the query has key-value pairs provided
          ...
          return select(table_name, columns, result_ids,  start, limit, sort, distinct);
        },

        // alias for query() that takes a dict of params instead of positional arguments
        queryAll: function(table_name, params) {
          if(!params) {
            ...
            return this.query(table_name,
              ...
              params.hasOwnProperty('columns') ? params.columns : null,
        ...
        function use(db_name) {
          ...
          if ( !( db && (db = JSON.parse(db)) && db.tables && db.data ) ) {
            if (!validateName(db_name)) {
              ...
              db = {tables: {}, data: {}, version : db_version};
              commit();
              db_new = true;
            }
        ...
        function drop() {
          ...
          if(localStorage.hasOwnProperty(db_id)) {
        ...
        function select(table_name, columns, ids, start, limit, sort, distinct) {
          ...
          for(var i = 0; i < ids.length; i++) {
            ...
            if (columns != null) {
              row = cloneWithAttributes(row, columns);
            }
        ...
        function queryByValues(table_name, data) {
          ...
          for(var ID in db.data[table_name]) {
            ...
            for(var field in data) {
              ...
              if(typeof data[field] === 'string' && row[field] != null) { // if the field is a string, do a case insensitive comparison
        ...
        function queryByFunction(table_name, query_function) {
          ...
          // loop t,rough all the records in the table, looking for matches
          for(var ID in db.data[table_name]) {
            if( !db.data[table_name].hasOwnProperty(ID) ) {
              continue;
            }
        ...
        // clone an object only using the given attributes
        function cloneWithAttributes(obj, attrs) {
          var new_obj = {};
